...
function applyFilters(query: any, filters: { ano: number | null; turno: number | null; cargo: string | null; municipio: string | null; partido: string | null }, table: 'votacao' | 'candidatos' | 'comparecimento') {
  ...
  if (table !== 'comparecimento') {
    ...
    if (filters.partido) query = query.eq('partido', filters.partido);
  }
...
export function useKPIs() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      // Count candidates
      let cq = supabase.from('bd_eleicoes_candidatos' as any).select('id', { count: 'exact', head: true });
      cq = applyFilters(cq, filters, 'candidatos');
      const { count: totalCandidatos } = await cq;

      // Total votes
      let vq = supabase.from('bd_eleicoes_votacao' as any).select('total_votos');
      vq = applyFilters(vq, filters, 'votacao');
      const { data: votosData } = await vq.limit(1000);
      ...
      const pageSize = 1000;
      while (true) {
        let q = supabase.from('bd_eleicoes_votacao' as any).select('total_votos').range(page * pageSize, (page + 1) * pageSize - 1);
      ...
      // Count elected
      let eq2 = supabase.from('bd_eleicoes_candidatos' as any).select('id', { count: 'exact', head: true }).ilike('situacao_final', '%ELEITO%').not('situacao_final', 'ilike', '%NÃO ELEITO%');
      eq2 = applyFilters(eq2, filters, 'candidatos');
      const { count: totalEleitos } = await eq2;

      // Comparecimento
      let compQ = supabase.from('bd_eleicoes_comparecimento' as any).select('eleitorado_apto, comparecimento');
      compQ = applyFilters(compQ, filters, 'comparecimento');
      const { data: compData } = await compQ.limit(1000);
      const totalApto = (compData || []).reduce((s: number, r: any) => s + (r.eleitorado_apto || 0), 0);
      const totalComp = (compData || []).reduce((s: number, r: any) => s + (r.comparecimento || 0), 0);
...
export function useTop10Votados() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      let q = supabase.from('bd_eleicoes_votacao' as any).select('nome_candidato, partido, total_votos').order('total_votos', { ascending: false }).limit(10);
      q = applyFilters(q, filters, 'votacao');
      const { data } = await q;
      return data || [];
    },
...
export function useVotosPorPartido() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      let q = supabase.from('bd_eleicoes_votacao' as any).select('partido, total_votos');
      q = applyFilters(q, filters, 'votacao');
      const { data } = await q.limit(1000);
      
      const map = new Map<string, number>();
      (data || []).forEach((r: any) => {
        const p = r.partido || 'OUTROS';
...
export function useComparecimentoPorAno() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      let q = supabase.from('bd_eleicoes_comparecimento' as any).select('ano, eleitorado_apto, comparecimento');
...
export function useEleitos() {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      let q = supabase.from('bd_eleicoes_candidatos' as any).select('*').ilike('situacao_final', '%ELEITO%').not('situacao_final', 'ilike', '%NÃO ELEITO%').order('nome_urna').limit(8);
...
export function useRanking(search: string, page: number, sortBy: string, sortAsc: boolean) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      let q = supabase.from('bd_eleicoes_votacao' as any).select('*', { count: 'exact' });
      q = applyFilters(q, filters, 'votacao');
...
export function useCandidato(id: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await (supabase.from('bd_eleicoes_candidatos' as any) as any).select('*').eq('id', parseInt(id)).single();
      return data;
    },
    enabled: !!id,
  });
...
export function useCandidatoVotos(nomeCandidato: string, ano: number) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await (supabase.from('bd_eleicoes_votacao' as any) as any).select('*').eq('nome_candidato', nomeCandidato).eq('ano', ano).order('total_votos', { ascending: false });
      return data || [];
    },
...
export function useCandidatoHistorico(nomeUrna: string, numeroUrna: number, partido: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await (supabase.from('bd_eleicoes_candidatos' as any) as any).select('*').or(`nome_urna.eq.${nomeUrna},nome_completo.eq.${nomeUrna}`).order('ano');
      return data || [];
    },
    enabled: !!nomeUrna,
  });
...
export function useMunicipios() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await (supabase.from('bd_eleicoes_votacao' as any) as any).select('municipio').limit(1000);
...
export function usePartidos() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data } = await (supabase.from('bd_eleicoes_votacao' as any) as any).select('partido').limit(1000);
      const unique = [...new Set((data || []).map((r: any) => r.partido).filter(Boolean))].sort();
      return unique as string[];
    },
  });
}
...
export function useCheckEmpty() {
  return useQuery({
    queryKey: ['checkEmpty'],
    queryFn: async () => {
      const { count } = await (supabase.from('bd_eleicoes_votacao' as any) as any).select('id', { count: 'exact', head: true });
      return (count || 0) === 0;
    },
  });
}
