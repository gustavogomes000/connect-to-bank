...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const tipo = 'partido'
    const tableName = 'bd_eleicoes_votacao_partido'
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      records.push({
        ...
        votos_nominais: getInt(row, 'QT_VOTOS_NOMINAIS_VALIDOS') || getInt(row, 'QT_VOTOS_NOMINAIS') || 0,
        votos_legenda: getInt(row, 'QT_VOTOS_LEGENDA') || 0,
        total_votos: getInt(row, 'QT_VOTOS_VALIDOS') || getInt(row, 'QT_VOTOS_NOMINAIS_VALIDOS') || 0,
      })
    }

    console.log(`Parsed ${records.length} registros partido GO`)
    ...
    try {
      ...
      if (ano) {
        await supabase.from('bd_eleicoes_importacoes_log').insert({
          ano, tipo: 'partido', status: 'erro', erro: error.message,
          finalizado_em: new Date().toISOString()
        })
      }
    } catch (_) {}
