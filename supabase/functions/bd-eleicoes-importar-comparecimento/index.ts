...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const tipo = 'comparecimento'
    const tableName = 'bd_eleicoes_comparecimento'

    await supabase.from('bd_eleicoes_importacoes_log').insert({
      ano, tipo, status: 'importando', iniciado_em: new Date().toISOString()
    })

    const url = `https://cdn.tse.jus.br/estatistica/sead/odsele/detalhe_votacao_munzona/detalhe_votacao_munzona_${ano}.zip`
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      records.push({
        ...
        votos_nominais: getInt(row, 'QT_VOTOS_NOMINAIS') || 0,
        votos_brancos: getInt(row, 'QT_VOTOS_BRANCOS') || 0,
        votos_nulos: getInt(row, 'QT_VOTOS_NULOS') || 0,
        votos_legenda: getInt(row, 'QT_VOTOS_LEGENDA') || 0,
      })
    }

    console.log(`Parsed ${records.length} registros comparecimento GO`)
    ...
    try {
      ...
      if (ano) {
        await supabase.from('bd_eleicoes_importacoes_log').insert({
          ano, tipo: 'comparecimento', status: 'erro', erro: error.message,
          finalizado_em: new Date().toISOString()
        })
      }
    } catch (_) {}
