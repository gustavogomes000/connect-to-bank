...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const tableName = 'bd_eleicoes_candidatos'
    ...
    const goFile = Object.keys(files).find(n =>
      (n.includes('_GO') || n.includes('_go')) && (n.endsWith('.csv') || n.endsWith('.txt'))
    )
    if (!goFile) throw new Error(`Arquivo GO não encontrado. Arquivos: ${Object.keys(files).join(', ')}`)

    const text = new TextDecoder('windows-1252').decode(files[goFile])
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      const sqCand = getVal(row, 'SQ_CANDIDATO')
      records.push({
        ...
        nome_partido: getVal(row, 'NM_PARTIDO'),
        numero_partido: getInt(row, 'NR_PARTIDO'),
        municipio: getVal(row, 'NM_MUNICIPIO'),
        codigo_municipio: getVal(row, 'CD_MUNICIPIO'),
        zona: getInt(row, 'NR_ZONA'),
        situacao_candidatura: getVal(row, 'DS_SITUACAO_CANDIDATURA'),
        situacao_final: getVal(row, 'DS_SIT_TOT_TURNO'),
        genero: getVal(row, 'DS_GENERO'),
        grau_instrucao: getVal(row, 'DS_GRAU_INSTRUCAO'),
        ocupacao: getVal(row, 'DS_OCUPACAO'),
        data_nascimento: getVal(row, 'DT_NASCIMENTO'),
        nacionalidade: getVal(row, 'DS_NACIONALIDADE'),
        sequencial_candidato: sqCand,
        foto_url: sqCand
          ? `https://divulgacandcontas.tse.jus.br/candidaturas/oficial/${getInt(row, 'ANO_ELEICAO') || ano}/${sqCand}/foto.jpeg`
          : null,
      })
    }

    console.log(`Parsed ${records.length} candidatos GO`)
    ...
    for (let i = 0; i < records.length; i += batchSize) {
      ...
      if (error) {
        ...
        else console.log(`Retry failed: ${e2.message}`)
      } else {
        inserted += batch.length
      }
      if (i + batchSize < records.length) await new Promise(r => setTimeout(r, 50))
    }
    ...
    try {
      ...
      if (ano) {
        await supabase.from('bd_eleicoes_importacoes_log').insert({
          ano, tipo: 'candidatos', status: 'erro', erro: error.message,
          finalizado_em: new Date().toISOString()
        })
      }
    } catch (_) {}
